% code adapted from https://github.com/smatmo/l0-sparse-NMF/blob/master/NMFL0_W.m
% Each row of the factor matrix can have only one non-zero value, to ensure that no 
% overlapping voxels among the latent factors were obtained by the decomposition
...
for iter=1:numIter
    ...
    for p=1:K
        [~, idx] = sort(W(:,p),'ascend');
        W(idx(1:D-L),p) = 0;
        W(:,p)=W(:,p)./max(max(W(:,p)),0.0000001);
    end

    % set 1 row only one voxel
   
    for p0=1:D
        [~, idx2] =sort(W(p0,:),'ascend');
        W(p0,idx2(1:K-1)) = 0;
    end
